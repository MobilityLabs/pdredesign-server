class V1::AssessmentsController < ApplicationController
  ...
  caches_action :index, :show, cache_path: { user_id: current_user.id }
  ...
  def create
    ...
    if @assessment.save
      expire_action action: :show, cache_path: { user_id: current_user.id }
      expire_action action: :index, cache_path: { user_id: current_user.id }
      render :show
    else
